...
func callGemini(ctx context.Context, prompt string) (string, error) {
	span, ctx := tracer.StartSpanFromContext(ctx, "call-gemini")

	span.SetTag("gemini.prompt", prompt)
	...
	span.SetTag("gemini.response", response)
	span.Finish()
	return response, nil
}
...
func callOllama(ctx context.Context, prompt string) (string, error) {
	span, ctx := tracer.StartSpanFromContext(ctx, "call-ollama")

	span.SetTag("ollama.prompt", prompt)
	requestBody := map[string]interface{}{
		"model":  "gemma3:270m",
		"prompt": prompt,
		"stream": false,
	...
	span.SetTag("ollama.response", response)
	span.Finish()
	return response, nil
}
